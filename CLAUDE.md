...
- Recent focus: Phase1 item #5 - MCP tool annotations (Python PR #551): `shared.ToolAnnotations` struct (authoring side) + `ToolOption`/`WithToolAnnotations` on `NewTool()`; `annotationsToMap` json round-trip; `buildToolsListResult` extracted from `routeMcpMethod` (gocyclo refactor, removed nolint); `tools/list` omits key for nil, emits empty map for `&ToolAnnotations{}`; example 14 demonstrates `WithToolAnnotations` on circle_area tool; Phase1 item #4 - 3 new hook events + missing fields (Python PR #545): Notification, SubagentStart, PermissionRequest event types; hook count 7 -> 10; PostToolUseFailure hook event (Python PR #535, Phase1 item #2); AssistantMessage error object form (Python PR #506, Go PR #124 squash); GetMcpStatus() control protocol method (Python PR #516, Go PR #124)
